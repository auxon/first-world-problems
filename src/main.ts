...
scene.fog = new THREE.Fog(0x1e1e32, 15, 32);

const camera = new THREE.PerspectiveCamera(50, window.innerWidth / window.innerHeight, 0.1, 100);
camera.position.set(6, 7, 9);
camera.lookAt(0, 1, 0);

// Lights — brighter for a less depressive first-world apartment
...
const fill = new THREE.DirectionalLight(0x90a0ff, 0.5);
fill.position.set(-4, 4, -3);
scene.add(fill);

const pointWarm = new THREE.PointLight(0xffaa66, 1.1, 14);
pointWarm.position.set(-2, 2.5, 1);
pointWarm.castShadow = true;
scene.add(pointWarm);

const pointCool = new THREE.PointLight(0x88aaff, 0.4, 10);
...
const floorMat = createMaterial(0x2a2a3a, 0.85);
...
function makeWall(w: number, h: number, d: number, x: number, y: number, z: number, rotY = 0) {
  ...
  const mat = createMaterial(0x3a3a4e, 0.9);
...
function animate() {
  ...
  if (direction.lengthSq() > 0) {
    ...
    character.position.x += moveX * 3.5 * dt;
    character.position.z += moveZ * 3.5 * dt;
